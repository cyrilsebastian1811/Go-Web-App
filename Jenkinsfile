    ...
    stage('Helm-Charts update') {
      steps {
        script{
          ...
          def presentVersion = sh(returnStdout: true, script: "yq r webapp-goapp/Chart.yaml version")
          ...
          sh "yq w -i webapp-goapp/Chart.yaml 'version' ${nextVersion}"
          sh "yq r webapp-goapp/Chart.yaml version"
          sh "yq w -i webapp-goapp/values.yaml 'dockerImage' ${REPOSITORY}:${git_hash}"
          sh "yq w -i webapp-goapp/values.yaml 'imageCredentials.registry' https://index.docker.io/v1/"
        }
      }
    }
        ...
        stage('Push to Helm-Charts Repo') {
            steps {
                script {
                    sshagent(['github-ssh']) {
                        sh "git commit -am 'goapp version upgrade to ${nextVersion} by jenkins'"
